...
def index(request):
    # Recupera todos os produtos publicados e em destaque.
    produtos = Produto.objects.filter(status_produto="published", destaque=True)
    # Recupera todos os vendedores.
    vendedores = Vendedor.objects.all()
    # Recupera todas as categorias e conta a quantidade de produtos em cada categoria usando a função annotate().
    categorias = Categoria.objects.all().annotate(produto_count=Count('categoria'))

    # Itera sobre cada produto para calcular a média das avaliações.
    for p in produtos:
        # Usa a função aggregate() para calcular a média das avaliações de cada produto.
        media_aval = AvaliacaoProduto.objects.filter(produto=p).aggregate(average_classification=Avg('classificacao'))
        # Verifica se existe alguma avaliação para o produto. Se sim, atribui a média das avaliações ao atributo media_avaliacoes do produto. Caso contrário, atribui 0.
        if media_aval['average_classification'] is not None:
            p.media_avaliacoes = media_aval['average_classification']
        else:
            p.media_avaliacoes = 0

        # Converte o campo badges (que pode ser uma string ou None) em uma lista. Isso corrige um problema de compatibilidade com o SQLite.
        p.badges = list(p.badges) if p.badges is not None else []

    # Inicializa a variável cart_total_amount com 0.
    cart_total_amount = 0
    # Verifica se existe um carrinho de compras na sessão do usuário.
    if 'cart_data_obj' in request.session:
        # Itera sobre os itens do carrinho de compras e calcula o valor total.
        for item in request.session['cart_data_obj'].values():
            # Utiliza try-except para tratar possíveis erros durante o cálculo do valor total.
            try:
                # Soma o valor total de cada item do carrinho.
                cart_total_amount += float(item['price']) * int(item['qty'])
            except (KeyError, ValueError, TypeError):
                # Ignora erros durante o cálculo (pode ser devido a dados corrompidos no carrinho).
                pass # Ignore errors during calculation

    # Cria um dicionário com os dados que serão passados para o template.
    context = {
        # Lista de produtos.
        "produtos": produtos,
        # Lista de vendedores.
        "vendedores": vendedores,
        # Lista de categorias com a contagem de produtos.
        "categorias": categorias,
        # Valor total do carrinho de compras.
        "cart_total_amount": cart_total_amount,
    }
    # Renderiza o template index.html com os dados do contexto.
...
def cart_view(request):
    ...
    if 'cart_data_obj' in request.session:
        ...
        return render(request, "core/cart.html", {
            ...
            'create_order_url': reverse('core:create_order'),
        })
    else:
